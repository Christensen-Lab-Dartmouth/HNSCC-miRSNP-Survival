...
dpi = 300
png("TCGA_miRseq_analysis/Survival/Data_files/TCGA_normal_tissue_expression_disrtibutions.png", width = dpi*9, height = dpi*8, res = dpi)
par(mfrow=c(1,2))
hist(avg_rpm$mean_rpm, main = "miR rpm averaged", xlab = "Average reads per million")
hist(log2(avg_rpm$mean_rpm), main = "miR rpm averaged log2", xlab = "Average reads per million")
dev.off()
